...
lambda_seq <- cate_fit$lasso_fit$lambda
lambda_seq <- lambda_seq[lambda_seq <= lambda_star]
# share seed across bootstrap replicates
seed <- rnorm(1)
nboot <- 200

estimates_seq <-lapply(lambda_seq, function(lambda) {
  cate_fit_lambda <- bootstrap_hal(cate_fit, lambda = lambda, nboot = 100, seed = seed)
  cbind(lambda, inference_delta_method(cate_fit_lambda, functional = functional_mean))
})
estimates_seq <- as.data.table(do.call(rbind, estimates_seq))
estimates_seq <- estimates_seq[!duplicated(estimates_seq$estimate),]
