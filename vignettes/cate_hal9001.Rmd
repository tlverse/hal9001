...
out <- rbindlist(lapply(1:100, function(iter) {
  ...
  cate_fit <- fit_hal_cate(X, Y, A, A.hat = plogis(X), Y.hat = X + plogis(X) * (1 + X),  smoothness_orders = 0, num_knots = 100)
  ...
  as.data.table(inference_delta_method(cate_fit, functional ))
}))
