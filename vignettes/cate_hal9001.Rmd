---
title: "Highly Adaptive Lasso Estimation of CATE and Bootstrap-Assisted Inference of Functionals"
output:
...
# Relaxed coefficient fit of original HAL.
head(bootstrapped_cate_fit$coefs)
```

We can perform bootstrapping based on an undersmoothed HAL fit by specifying a regularization parameter, lambda, that is less than the CV-selected one, lambda_star. Undersmoothing can enhance the performance of inferences based on the bootstrap. Furthermore, we can employ the plateau method for data-adaptive selection of lambda, as described in "Nonparametric Bootstrap Inference for the Targeted Highly Adaptive LASSO Estimator" by Ca and van der Laan (2019). While the plateau method for automatic targeted undersmoothing is not yet supported natively in hal9001, it will be included in a future release.

```{r}

cate_fit <- fit_hal_cate(X, Y, A,  smoothness_orders = 1, num_knots = 100)
bootstrapped_cate_fit_lambda <- bootstrap_hal(cate_fit, lambda = 0.1*cate_fit$lambda_star)
head(bootstrapped_cate_fit$coefs)
```
...
```{r}
out <- as.data.table(inference_pointwise(bootstrapped_cate_fit, new_data = as.matrix(X)))
head(out)
...
### (Experimental, in progress) Targeted undersmoothing with the bootstrap
...
m.true <- 2*X^2 + pi.true * (1 + abs(X) + 0.5*sin(4*X))
# true CATE
cate.true <-  (1  + 0.5*sin(5*X))
ATE.true <- 1.5
A <- rbinom(n, 1, pi.true)
# Robinson's parameterization of outcome regression
mu.true <- m.true + (A - pi.true) * cate.true
Y <- rnorm(n, mu.true, 0.5)


functional_mean <- function(hal_fit, X, X_unpenalized = NULL, offset = NULL, other_data = NULL, ...) {
   print(length(hal_fit$basis_list))
   print(dim(hal_fit$coefs))
  mean(predict(hal_fit, X, X_unpenalized, offset))
...
library(data.table)
library(ggplot2)
cate_fit <- fit_hal_cate(X, Y, A,  smoothness_orders = 0, num_knots = 50)
# get cv-selected lambda
lambda_star <- cate_fit$lambda_star
# get lambda sequence that are smaller than cv-selector (undersmoothed)
lambda_seq <- sort(cate_fit$lasso_fit$lambda, decreasing = TRUE)
lambda_seq <- lambda_seq[lambda_seq <= lambda_star]
# share seed across bootstrap replicates
seed <- rnorm(1)
nboot <- 100
ell_1norm_star = sum(abs(cate_fit$coefs[-1]))
stop_flag <- FALSE
estimates_list <-lapply(lambda_seq, function(lambda) {
  print(lambda)
  out <- NULL
  try({
  cate_fit_lambda <- bootstrap_hal(cate_fit, lambda = lambda, nboot = nboot, seed = seed)
  ell_1norm = sum(abs(cate_fit_lambda$coefs[-1]))
  out <- cbind(lambda, ell_1norm, inference_delta_method(cate_fit_lambda, functional = functional_mean))
  })
  return(out)
})
radius_monotone <- as.stepfun(isoreg(estimates_seq$ell_1norm, (estimates_seq$CI_upper - estimates_seq$CI_lower)/2))(estimates_seq$ell_1norm)
estimates_seq$CI_lower_iso <- estimates_seq$estimate - radius_monotone
estimates_seq$CI_upper_iso <- estimates_seq$estimate + radius_monotone

estimates_seq <- as.data.table(do.call(rbind, estimates_list))
estimates_seq <- estimates_seq[!duplicated(estimates_seq[, c("ell_1norm"), with = FALSE]),]
 
#estimates_seq$CI_lower <- estimates_seq$estimate - radius_monotone
#estimates_seq$CI_upper <- estimates_seq$estimate + radius_monotone
estimates_seq$radius_monotone <- radius_monotone

 

ggplot(estimates_seq, aes(x = ell_1norm, y = estimates_seq$CI_upper - estimates_seq$CI_lower)) + 
  geom_line() +  # Add the line for estimates
  #geom_ribbon(aes(ymin = CI_lower, ymax = CI_right), alpha = 0.2) +  # Add shaded confidence intervals
  labs(x = "Ell-1 norm", y = "CI radius", title = "CI radius vs. ell-1 norm with Confidence Intervals") +
  theme_bw() + scale_x_log10()


# ggplot(estimates_seq, aes(x = lambda, y =  CI_lower)) + 
#   geom_line() +  # Add the line for estimates
#   #geom_ribbon(aes(ymin = CI_lower, ymax = CI_right), alpha = 0.2) +  # Add shaded confidence intervals
#   labs(x = "Lambda", y = "Lower CI", title = "LowerCI vs. Lambda with Confidence Intervals") +
#   theme_bw() + scale_x_log10()

cate_fit_lambda <- bootstrap_hal(cate_fit, lambda = max(lambda_seq))
plot(X, predict(cate_fit_lambda, X))
cate_fit_lambda <- bootstrap_hal(cate_fit, lambda = min(lambda_seq))
plot(X, predict(cate_fit_lambda, X))
```
...
data_plot <- data.frame(X = X, cate_hat = cate.hat, cate_true = cate.true)
data_plot <- data_plot[order(data_plot$X),]
ggplot(data_plot, aes(x = X)) +
  geom_line(aes(y = cate_hat), color = "blue", size = 1) +
  geom_line(aes(y = cate_true), color = "black", alpha = 0.5, size = 1) +
  labs(x = "Covariate (X)", y = "Estimated CATE") +
  theme_bw()
ggsave("demo_cate_plot.pdf")
...
out <- as.data.table(inference_delta_method(
