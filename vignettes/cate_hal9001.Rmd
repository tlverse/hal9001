...
### Targeted undersmoothing with the bootstrap
...
estimates_list <-lapply(lambda_seq, function(lambda) {
  ...
  cbind(ell_1norm, inference_delta_method(cate_fit_lambda, functional = functional_mean))
  })
})
estimates_seq <- as.data.table(do.call(rbind, estimates_list))
estimates_seq <- estimates_seq[!duplicated(estimates_seq[, c("estimate", "CI_lower"), with = FALSE]),]
