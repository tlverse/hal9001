...
## Bootstrap-assisted inference for ATE using HAL

In causal inference problems, estimates and inference for functionals of the CATE, such as the average treatment effect (ATE), are often required. The `hal9001` package implements a bootstrap-assisted inference method that provides confidence intervals for functionals of HAL fits. To utilize this method, we first need to bootstrap the HAL fit. It is important to note that our bootstrap procedure does not re-bootstrap the HAL lasso problem itself; rather, it only refits the coefficients of the basis functions selected by HAL using bootstrap replicates of the data with unpenalized generalized linear models. As a result, bootstrapping is computationally inexpensive and runs quickly. 

To bootstrap, we simply pass our fitted HAL object to the `bootstrap_hal` function, which by default uses `nboot = 1000` bootstrap replicates. The output of this function is a nearly identical HAL fit object, except with model coefficients refit without penalization, and a list of HAL fit objects corresponding to each bootstrap iteration. For inference, we then pass our bootstrapped HAL object to the `inference_delta_method` function, specifying a functional of interest for inference. This functional must be a user-specified function that accepts the arguments `hal_fit, X, X_unpenalized, offset, other_data`, where `other_data` is an optional user-specified matrix that can be supplied through the `inference_delta_method` function (and will be bootstrapped as well).
...
library(data.table)
out <- rbindlist(lapply(1:100, function(iter) {
