...
HAL may also be fit through a call to the popular `glmnet` R package:

```{r fit-hal-glmnet}
hal_fit2 <- fit_hal(X = x, Y = y, fit_type = "glmnet")
hal_fit2$times
```
...
```{r fit-hal-reduced}
hal_fit3 <- fit_hal(X = x, Y = y, fit_type = "lassi", reduce_basis = 0.2)
hal_fit3$times
```

In the above, all basis functions with fewer than 20% of observations meeting
the criterion imposed are automatically removed prior to the Lasso step of
fitting the HAL regression.
...
preds_hal <- predict(object = hal_fit1, new_data = x)
mse_hal <- mse(preds = preds_hal, y = y)
mse_hal
```

```{r eval-oob}
oob_hal <- predict(object = hal_fit1, new_data = test_x)
