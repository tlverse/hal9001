...
# get formula object
fit <- fit_hal(
  X = X, Y = Y, formula = ~ h(.), smoothness_orders = 1, num_knots = 100, screen_variables = FALSE,
)
...
Y <- rnorm(n, mu, 0.5)
out <- fit_hal(X, Y, max_degree = 1, num_knots = 10, screen_variables = TRUE, screen_interactions = TRUE) # Should take no more than a minute
