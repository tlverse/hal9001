...
This can be done using the `smoothness_order` argument. Setting
`smoothness_order = 0` gives a piece-wise constant fit (via zero-order basis
...
Setting `smoothness_order = 1` gives a piece-wise linear fit (via first-order
basis functions), which is continuous and mostly differentiable. In general,
`smoothness_order = k` corresponds to a piece-wise polynomial fit of degree
$k$. Mathematically, `smoothness_order = k` corresponds with finding the best
...
hal_fit_0 <- fit_hal(
  X = x, Y = y, smoothness_order = 0, num_knots = num_knots
)

hal_fit_smooth_1 <- fit_hal(
  X = x, Y = y, smoothness_order = 1, num_knots = num_knots
)

hal_fit_smooth_2_all <- fit_hal(
  X = x, Y = y, smoothness_order = 2, num_knots = num_knots, 
...
hal_fit_0 <- fit_hal(
  X = x, Y = y, smoothness_order = 0, num_knots = 300
)
hal_fit_smooth_1 <- fit_hal(
  X = x, Y = y, smoothness_order = 1, num_knots = 300
)

hal_fit_smooth_2 <- fit_hal(
  X = x, Y = y, smoothness_order = 2, num_knots = 300
)
...
rm(smoothness_order)
rm(num_knots)
# `h` excepts the arguments `s` and `k`. `s` stands for smoothness and is equivalent to smoothness_order in use. `k` specifies the number of knots. ` 
...
formula <- formula_hal(
  ~ h(x1) + h(x2) + h(A), X = X, smoothness_order = 1, num_knots = 10
)
# Note that the arguments smoothness_order and/or num_knots will not be used if `s` and/or `k` are specified in `h`.
formula <- formula_hal(
  ~ h(x1, k=1) + h(x2,  k=1) + h(A, k=1), X = X, smoothness_order = 1, num_knots = 10
)
...
these constraints is achieved by specifying the `monotone` argument of `h`. Note if smoothness_order = 0 then this is a monotonicity constrain on the function, but if if smoothness_order = 1 then this is a monotonicity constraint on the function's derivative (e.g. a convexity constraint). We can also specify that certain terms are not penalized in the LASSO/glmnet using the `pf` argument of `h` (stands for penalty factor).

```{r}
# An additive monotone increasing model
formula <- formula_hal(
  y ~ h(., monotone = "i"), X, smoothness_order = 0, num_knots = 100
)
...
formula <- formula_hal(
  y ~ h(., monotone = "i", pf = 0), X, smoothness_order = 0, num_knots = 100
)
...
formula <- formula_hal(
  ~ h(., monotone = "d") + h(.,., monotone = "d"), X, smoothness_order = 1, num_knots = 100
)
...
# get formula object
fit <- fit_hal(
  X = X, Y = Y, formula = ~ h(.), smoothness_order = 1, num_knots = 100
)
