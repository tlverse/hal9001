...
cv_coxnet <- cv.glmnet(x = x_surv, y = y_surv, family = "cox", maxit = 1000)
coxnet_pred <- as.numeric(predict(cv_coxnet, x_surv, type = "response"))

# try with hal9001 instead of glmnet
cv_halcox <- fit_hal(X = x_surv, Y = y_surv, fit_type = "glmnet",
                     family = "cox", max_degree = NULL, yolo = FALSE)
...
nocv_halcox <- fit_hal(X = x_surv, Y = y_surv, fit_type = "glmnet",
                       family = "cox", max_degree = NULL, cv_select = FALSE,
                       nlambda = 200, yolo = FALSE)
