...
test_that("coordinate descent works as it does in R", expect_equivalent(
  ls_beta,
  beta_new
))
...
test_that(
  "the updated residuals are as expected",
...
# library(ggplot2)
# combined=data.frame(lambda=1:100, mse=c(mses,gmses), type=rep(c("lassi","glmnet"),each=100))
