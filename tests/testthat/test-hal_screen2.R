...
goodbasis <- hal_screen_goodbasis(x, y, actual_max_degree = 2, k = NULL, family = 'gaussian')
...
test_that("Goodbasis function works properly with interaction", {
  ...
  expect_setequal(x_basis_str, goodbasis_str)#when k=6, they must be equal, all columns would be selected
})
# 
