# hal implementation of duplicate detection for comparision purposes
#' @import data.table
os_find_dupes <- function(X.init) {
    ...
    # Pre-allocate a data.table with one column, each row will store a single column
    # from X.init
    datDT <- data.table(ID = 1:nIndCols, bit_to_int_to_str = rep.int("0", nIndCols))
    # Each column in X.init will be represented by a unique vector of integers.  Each
    # indicator column in X.init will be converted to a row of integers or a string
    # of cat'ed integers in data.table. The number of integers needed to represent a
    # single column is determined automatically by package 'bit' and it depends on
    # nrow(X.init)
    ...
    for (i in 1:nIndCols) {
        ...
        # print(i); print(nonzero_rows) X.init@i[i:X.init@p[i]]+1 # row numbers of
        ...
        data.table::set(datDT, i, 2L, value = stringr::str_c(stringr::str_replace_na(intval), 
            collapse = ""))
    }
    ...
    # add logical column indicating duplicates, following the first non-duplicate
    # element
