...
#'
#'  For \code{smoothness_orderss = 0}, only zero-order splines (piece-wise
#'  constant) are generated, which assume the true regression function has no
#'  smoothness or continuity. When \code{smoothness_orderss = 1}, first-order
#'  splines (piece-wise linear) are generated, which assume continuity of the
#'  true regression function. When \code{smoothness_orderss = 2}, second-order
...
#'  considering \code{smoothness_orderss = 0}, too few knot points (e.g., < 50)
#'  can significantly reduce performance. When \code{smoothness_orderss = 1} or
...
#'  \code{smoothness_orderss}, and as a vector of length \code{max_degree} with
...
#'  of \code{num_knots} follows this logic --- for \code{smoothness_orderss = 0},
#'  \code{num_knots} is set to \eqn{500 / 2^{j-1}}, and for
#'  \code{smoothness_orderss = 1} or higher, \code{num_knots} is set to
...
#'    - If \code{smoothness_orderss = 0} and \code{max_degree = 3},
#'      \code{num_knots = c(400, 200, 100)}.
#'    - If \code{smoothness_orderss = 1+} and \code{max_degree = 3},
...
#' @param smoothness_orderss An \code{integer}, specifying the smoothness of the
#'  basis functions. See details for \code{smoothness_orderss} for more
#'  information.
...
#'  \code{max_degree} and \code{smoothness_orderss}, which prevents (expensive)
...
fit_hal <- function(X,
                    ...
                    smoothness_orderss = 1,
                    num_knots = num_knots_generator(
                      max_degree = max_degree,
                      smoothness_orderss = smoothness_orderss,
  ...
  if (!is.null(formula)) {
    # formula <- formula_hal(
    #   formula = formula, X = X, smoothness_ordersss = smoothness_ordersss,
    ...
    if(!inherits(formula, "formula_hal")) {
      formula <- formula_hal(formula, X = X, smoothness_orderss = smoothness_orderss, num_knots = num_knots)
    }
  ...
  if (is.null(basis_list)) {
    basis_list <- enumerate_basis(
      X,
      max_degree = max_degree,
      smoothness_ordersss = smoothness_orderss,
      num_knots = num_knots,
...
#' @param smoothness_orderss see \code{\link{fit_hal}}.
...
#'
#' @keywords internal
num_knots_generator <- function(max_degree, smoothness_orderss, base_num_knots_0 = 500,
