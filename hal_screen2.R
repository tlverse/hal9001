...
hal_screen_goodbasis<-function(x, y, actual_max_degree, k = NULL, family, col_lists = NULL, foldid = NULL, offset = NULL, verbose = FALSE){
  ...
  if (actual_max_degree >= 2) {
    ...
    x_basis_lists<-as.list(matrix(0,ncol = length(col_lists)+length(interaction_col_lists)))
    ...
    screened_rank <- hal_screen_rank(x_interaction_basis, y, k = k,
                                     family = family,
                                     foldid = foldid,
                                     offset = offset)
    ...
    set_mainterm <- list()
    for(i in 1:length(screened_col)){
      if (length(screened_col[[i]])!=1){
        set_interaction <- c(set_interaction, as.list(screened_col[[i]]))
      }
      else{set_mainterm <- c(set_mainterm, as.list(screened_col[[i]]))}#get set of main terms
    }
    set_interaction <- set_interaction[!duplicated(set_interaction)]#get set of main terms that build all the interaction
    screened_col <- c(screened_col, setdiff(set_interaction, set_mainterm))#include all the main terms that build the interaction terms
  }
  else {screened_rank <- hal_screen_rank(x, y, k = k,
                                         family = family,
                                         foldid = foldid,
                                         offset = offset)
